明白了，我帮你把完整内容整理成 **纯文本形式**，去掉代码块标记和多余缩进，这样你可以直接全部复制到 `README.md` 里。
...
打开 [http://localhost:3000/settings，点击任意模型提供方卡片：](http://localhost:3000/settings，点击任意模型提供方卡片：)

1. 填写 API Key
2. 点击连通性检测验证配置
3. 点击获取模型拉取官方模型列表，或手动新建模型

### 2. 开始对话

打开 [http://localhost:3000/chat：](http://localhost:3000/chat：)
...
在 [http://localhost:3000/settings](http://localhost:3000/settings) 的「对话设置」部分可调整：
...
你可以直接复制上面全部内容到本地 `README.md` 文件，然后保存。这样就不会有任何冲突标记了。

如果你希望，我可以帮你生成一个 **PowerShell 一键上传脚本**，把本地整个项目和这个新的 README 一起上传 GitHub，一次完成。
